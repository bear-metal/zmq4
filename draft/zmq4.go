...
func (soc *Socket) SendBytes(data []byte, flags Flag, options ...interface{}) (int, error) {
	...
	var msg C.zmq_msg_t
	rc, err := C.zmq4_msg_init_size(&msg, C.size_t(len(data)))
	if rc != 0 {
	...
	for _, option := range options {
		...
		case OptRoutingId:
			...
			rc, err := C.zmq4_msg_set_routing_id(&msg, id)
			if rc != 0 {
				return int(rc), errget(err)
			}
		case OptGroup:
			s := C.CString(string(t))
			rc, err := C.zmq4_msg_set_group(&msg, s)
	...
	var size C.int
	for {
		size, err = C.zmq4_msg_send(&msg, soc.soc, C.int(flags))
		if size >= 0 || !soc.ctx.retry(err) {
			break
		}
	}
